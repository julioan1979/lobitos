...
def dashboard_tesoureiro(dados: dict):
    ...
    # Preparar um DataFrame limpo mesmo quando não há dados, para garantir
    # que a secção da "Posição de Caixa" e as métricas são sempre executadas.
    ...
    if colunas_existentes:
        ...
        # fallback: DataFrame vazio com colunas finais esperadas
        df_rec_limpo = pd.DataFrame(columns=["Escuteiro", "Valor (€)", "Meio de Pagamento", "Data", "Quem Recebeu"])
    ...
    df_rec_limpo = df_rec_limpo.rename(columns={
        ...
        # Exibe sempre o período selecionado
        st.caption(
            f"Período selecionado: {data_inicio_ts.strftime('%d/%m/%Y')} - {data_fim_ts.strftime('%d/%m/%Y')}"
        )

        # Mostrar métricas sempre, garantindo valores zero quando não há movimentos
        # Os labels de "Nenhum recebimento/estorno no período" já indicam a ausência de movimentos
        total_recebimentos = total_recebimentos_periodo if (isinstance(df_rec_periodo, pd.DataFrame) and "Valor (€)" in df_rec_periodo.columns) else 0.0
        total_estornos = total_estornos_periodo if (isinstance(df_estornos_periodo, pd.DataFrame) and "Valor (€)" in df_estornos_periodo.columns) else 0.0
