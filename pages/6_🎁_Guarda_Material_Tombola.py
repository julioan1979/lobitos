...
def _render_ver_stock() -> None:
    st.subheader("📦 Ver stock")
    ...
    if not df_inv.empty and "CaixaAtual" in df_inv.columns:
        itens_sem_caixa = int(df_inv["CaixaAtual"].apply(lambda v: not (isinstance(v, list) and len(v) > 0)).sum())
    ...
    if baixo_stock > 0:
        st.warning(f"Alerta: existem {baixo_stock} itens em baixo stock (<=2).")
    if itens_sem_caixa > 0:
        st.warning(f"Aviso: existem {itens_sem_caixa} itens sem caixa atribuída.")

    if df_inv.empty:
        st.info("Ainda não existem itens no inventário.")
    else:
...
def _render_receber_doacao() -> None:
    st.subheader("🎁 Receber doação")
    ...
    st.markdown("**1) Criar patrocínio pendente**")
    ...
    st.markdown("**2) Processar e converter em stock**")
...
def _render_preparar_evento() -> None:
    st.subheader("🎪 Preparar evento")

    st.markdown("**1) Criar evento**")
    ...
    st.markdown("**2) Registar saída de material**")
...
def render_operacional() -> None:
    st.markdown("### Operações principais")
    aba_receber, aba_evento, aba_stock = st.tabs([
        "🎁 Receber doação",
        "🎪 Preparar evento",
        "📦 Ver stock",
    ])
    ...
    with aba_receber:
        _render_receber_doacao()
    with aba_evento:
        _render_preparar_evento()
    with aba_stock:
        _render_ver_stock()


def render_ferramentas_avancadas() -> None:
    tabs = st.tabs([
    ...
    aba_inv, aba_importacao, aba_caixas, aba_patrocinadores, aba_resumo = tabs
    ...
    with aba_caixas:
        ...
        df_caixas = _table_df(_table_ref("CAIXAS"))
        if df_caixas.empty:
            st.info("Ainda não existem caixas registadas.")
        else:
    ...
    with aba_patrocinadores:
        ...
        df_patrocinadores = _table_df(_table_ref("PATROCINADORES"))
        if df_patrocinadores.empty:
            st.info("Ainda não existem patrocinadores registados.")
        else:
    ...
    with aba_resumo:
        df_inv = _table_df(_table_ref("INVENTARIO"))
        st.subheader("Resumo rápido do inventário")
        total_itens_inv = len(df_inv.index)
        stock_total_inv = pd.to_numeric(df_inv.get("QuantidadeAtual"), errors="coerce").fillna(0).sum() if not df_inv.empty else 0
        itens_sem_caixa_inv = 0
        if not df_inv.empty and "CaixaAtual" in df_inv.columns:
            itens_sem_caixa_inv = int(df_inv["CaixaAtual"].apply(lambda v: not (isinstance(v, list) and len(v) > 0)).sum())

        col_r1, col_r2, col_r3 = st.columns(3)
        col_r1.metric("Itens no inventário", int(total_itens_inv))
        col_r2.metric("Stock total", int(stock_total_inv))
        col_r3.metric("Itens sem caixa", int(itens_sem_caixa_inv))
