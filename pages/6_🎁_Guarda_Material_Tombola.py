...
bootstrap_key = f"tombola_schema_bootstrap_{BASE_ID}"
if not st.session_state.get(bootstrap_key):
    with st.spinner("A preparar estrutura da base Tômbola..."):
        report_bootstrap = ensure_tombola_base_schema(api, BASE_ID)
    st.session_state[bootstrap_key] = report_bootstrap
else:
    report_bootstrap = st.session_state.get(bootstrap_key, {"created_tables": [], "created_fields": [], "warnings": []})

if report_bootstrap.get("created_tables") or report_bootstrap.get("created_fields"):
    st.success("Estrutura da base Tômbola validada/atualizada automaticamente pelo backend.")

if report_bootstrap.get("warnings"):
    with st.expander("Avisos de bootstrap da base Tômbola"):
        for aviso in report_bootstrap["warnings"]:
            st.warning(aviso)
